...
public class Exercise9Test extends ClassicOnlineStore {
    ...
    public void simplestStringJoin() {
        ...
        String nameAsCsv = customerList.stream()
    ...
    public void mapKeyedByItems() {
        ...
        BiConsumer<Map<String, Set<String>>, Customer> accumulator = (map, customer) -> customer.getWantToBuy()
            .forEach(item -> {
                if (map.containsKey(item.getName())) {
                    map.get(item.getName())
                        .add(customer.getName());
                } else {
                    map.put(item.getName(), Stream.of(customer.getName())
                        .collect(Collectors.toSet()));
                }
            });
        BinaryOperator<Map<String, Set<String>>> combiner = (left, right) -> {
            right.forEach((item, customers) -> left.merge(item, customers, (oldVal, newVal) -> {
        ...
        Collector<Customer, ?, Map<String, Set<String>>> toItemAsKey = new CollectorImpl<>(supplier, accumulator, combiner, finisher, EnumSet.of(Collector.Characteristics.CONCURRENT, Collector.Characteristics.IDENTITY_FINISH));
    ...
    public void bitList2BitString() {
        ...
        Collector<String, ?, String> toBitString = new Collector<String, List<Integer>, String>() {
            ...
            public BiConsumer<List<Integer>, String> accumulator() {
                return (list, str) -> {
                    ...
                    List<Integer> splitInt = splitString.stream()
                        .map(Integer::valueOf)
            ...
            public Function<List<Integer>, String> finisher() {
                return list -> {
                    ...
                    return list.stream()
                        .distinct()
                        .collect(new Collector<Integer, List<String>, String>() {
                            @Override
        ...
        String bitString = Arrays.stream(bitList.split(","))
            .collect(toBitString);
